...
echo "⏰ Seeding recurring jobs (Solid Queue) on ECS..."
...
# Run seed recurring task
echo "📋 Starting seed recurring jobs task..."
TASK_ARN=$(aws ecs run-task \
  --cluster $CLUSTER_NAME \
  --task-definition $TASK_DEFINITION \
  --launch-type FARGATE \
  --network-configuration "awsvpcConfiguration={subnets=[$SUBNETS],securityGroups=[$SECURITY_GROUPS],assignPublicIp=DISABLED}" \
  --overrides '{"containerOverrides":[{"name":"agent-marketing","command":["bundle","exec","rake","solid_queue:seed_recurring"]}]}' \
  --region $AWS_REGION \
...
if [ "$EXIT_CODE" == "0" ]; then
  echo "✅ Recurring jobs seeded successfully!"
  echo ""
  echo "The following jobs are now scheduled:"
  echo "  • scheduled_task_dispatcher - Every minute"
  echo "  • process_drip_campaigns - Every hour"
  echo "  • visualization_refresh - Every 15 minutes"
  echo "  • cleanup_temporary_uploads - Every hour"
  echo "  • agent_energy_regeneration - Every hour"
  echo "  • nightly_learning - Daily at 2 AM"
  echo "  • memory_cleanup - Daily at 3 AM"
else
  echo "❌ Seeding recurring jobs failed."
