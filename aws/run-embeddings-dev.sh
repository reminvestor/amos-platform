...
echo "🧠 Running embeddings update on ECS (DEV environment)..."
...
TASK_ARN=$(aws ecs run-task \
  ...
  --overrides "{\"containerOverrides\":[{\"name\":\"$CONTAINER_NAME\",\"command\":[\"bundle\",\"exec\",\"rails\",\"embeddings:update_all\"]}]}" \
  --region $AWS_REGION \
  --query 'tasks[0].taskArn' \
  --output text)

echo "🚀 Embeddings task started: $TASK_ARN"
echo "⏳ Waiting for embeddings update to complete..."
...
if [ "$EXIT_CODE" -eq "0" ]; then
  ...
  echo "❌ Embeddings update failed with exit code: $EXIT_CODE"
