...
#
# Usage:
#   AgentTrainingJob.perform_later(agent.id)
#   AgentTrainingJob.perform_later(agent.id, phases: ['research', 'knowledge_building'])
#
class AgentTrainingJob < ApplicationJob
  queue_as :default
  ...
  def perform(agent_id, options = {})
    ...
    Rails.logger.info "[AgentTrainingJob] Starting training for agent: #{agent.name}"
    ...
    if options[:phases].present?
      ...
      options[:phases].each do |phase|
        Rails.logger.info "[AgentTrainingJob] Running phase: #{phase}"
      ...
      Rails.logger.info "[AgentTrainingJob] Training complete: #{result.inspect}"
      
      # Notify if there's a notification system
      notify_training_complete(agent, result) if result[:success]
    end
  rescue => e
    Rails.logger.error "[AgentTrainingJob] Training failed for agent #{agent_id}: #{e.message}"
  ...
  def notify_training_complete(agent, result)
    ...
    Rails.logger.debug "[AgentTrainingJob] Could not notify: #{e.message}"
  end
end
