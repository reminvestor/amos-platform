class ProcessCampaignJob < ApplicationJob
  ...
  def perform(campaign_id, batch_size = 200, correlation_id = nil)
    Rails.logger.tagged("ProcessCampaignJob", "Campaign##{campaign_id}", correlation_id) do
      ...
      if campaign.status == 'scheduled'
        ...
        unless campaign.update(status: 'in_progress')
          ...
          Rails.logger.error("Failed to update campaign status to in_progress: #{error_message}")
      ...
      Rails.logger.info("Campaign batch processed: ID=#{campaign.id}, Sent=#{sent_count}, Remaining=#{pending_count}")
      ...
      if pending_count > 0
        ...
        unless campaign.update(status: 'completed')
          error_message = campaign.errors.full_messages.join(', ')
          Rails.logger.error("Failed to update campaign status to completed: #{error_message}")
          raise ActiveRecord::RecordInvalid, "Failed to update campaign: #{error_message}"
        end
      end
      
      Rails.logger.info("ProcessCampaignJob finished for campaign #{campaign_id}")
    end
  end
end
