class TaskModeDetector
  ...
  def determine_mode_and_confidence(interactive, autonomous, hybrid, message)
    # Handle edge cases - short messages default to interactive for safety
    if message.length < 10
      # Check if it's a known greeting/acknowledgment
      if message.match?(/\b(ok|okay|yes|no|bye|thanks|hello|hi)\b/i)
        return [ "autonomous", 0.6 ]
      end
      # Default vague short messages to interactive (safer)
      return [ "interactive", 0.3 ]
    end
