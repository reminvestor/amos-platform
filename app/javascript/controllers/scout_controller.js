...
export default class extends Controller {
  ...
  async processMessage(message, files = []) {
    try {
      ...
      // Reset thinking state - no indicator until tools actually run
      this.hideToolThinking(0)  // Force cleanup any existing
      this.currentStreamingContent = undefined  // Reset for fresh start
      this.streamingMessageElement = null
      this.thinkingDelayTimeout = null  // No automatic thinking indicator
    ...
    window.requestBrowserSessionSyncLogin = window.requestBrowserSessionSyncLogin || ((proxySessionId, proxyHost, currentUrl = "") => {
      ...
      fetch("/scout/browser_session_sync_proxy", {
        ...
        .catch((err) => {
          ...
          window.showError("Failed to sync login session")
        })
    })
